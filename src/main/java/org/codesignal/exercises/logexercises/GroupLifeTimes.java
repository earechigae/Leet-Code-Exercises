...
/*
Introduction
    Welcome to our new coding practice lesson!
    We have an interesting problem in this unit that centers around data from a social networking app.
    The challenge involves processing logs from this app and extracting useful information from them.
    This task will leverage your skills in string manipulation, working with timestamps, and task subdivision.
    Let's get started!

Task Statement

Imagine a social networking application that allows users to form groups. Each group has a unique ID ranging from 1 to n,
where n is the total number of groups. Interestingly, the app keeps track of when a group is created and deleted,
logging all these actions in a string.

The task before us is to create a Java method named analyzeLogs.
This method will take as input a string of logs and output a List<String> representing the groups with the longest lifetime.
Each string in the list contains two items separated by a space: the group ID and the group's lifetime.
By 'lifetime,' we mean the duration from when the group was created until its deletion.
If a group has been created and deleted multiple times, the lifetime is the total sum of those durations.
If multiple groups have the same longest lifetime, the method should return all such groups in ascending order of their IDs.
